...
class Serial_Link {
 public:
  ...
  Matrixf<_n, 1>  ikine_smooth(Matrixf<4, 4> Td, Matrixf<_n, 1> q_prev, float tol = 1e-4f, uint16_t max_iter = 50) {
  Matrixf<_n, 1> q = q_prev; // 用上一时刻状态初始化
  Matrixf<_n, _n> I = matrixf::eye<_n, _n>();
  float alpha = 0.01f; // 零空间优化权重
  float lambda = 0.2f; // 正则化参数
  float dq_max = 0.05f; // 最大关节变化（弧度）
  for (int i = 0; i < max_iter; i++) {
    Matrixf<4, 4> T = fkine(q);
    Matrixf<3, 1> pe = t2p(Td) - t2p(T);
    Matrixf<3, 1> we = t2twist(Td * invT(T)).block<3, 1>(3, 0);
    Matrixf<6, 1> err;
    for (int j = 0; j < 3; j++) {
      err[j][0] = pe[j][0];
      err[j + 3][0] = we[j][0];
    }
    if (err.norm() < tol) return q;
    Matrixf<6, _n> J = jacob(q);
    // 动态正则化
    // SVD<6, _n> svd(J);
    // float min_singular_value = svd.singularValues().min();
    // lambda = min_singular_value < 0.1f ? 0.5f : 0.01f; // 动态调整
    Matrixf<_n, 1> k = -2.0f * (q - q_prev); // 优化目标
    Matrixf<_n, _n> JTJ = J.trans() * J;
    Matrixf<_n, 1> dq = matrixf::inv(JTJ + lambda * I) * (J.trans() * err + alpha * k);
    // 限制dq幅度
    for (int j = 0; j < _n; j++) {
      dq[j][0] = math::limit(dq[j][0], -dq_max, dq_max);
    }
    q += dq;
    for (int j = 0; j < _n; j++) {
      if (links_[j].type() == robotics::Joint_Type_e::R) {
        q[j][0] = math::loopLimit(q[j][0], -PI, PI);
        q[j][0] = math::limit(q[j][0], links_[j].qmin(), links_[j].qmax());
      }
    }
    // 步长调整逻辑（可保留原逻辑）
  }
