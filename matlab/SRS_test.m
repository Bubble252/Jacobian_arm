...
trplot(Td, 'frame', 'T_d', 'color', 'red', 'length', 0.3,'arrow', 'linewidth', 2);  % 增加轴长和线宽
pos_errors = [];
att_errors = [];
dq_norms = [];
...
function srs = create_srs()
    % 创建一个典型的7自由度机械臂模型（带角度限制）
    % 关节参数: (d, a, alpha, 角度限制范围)
    L(1) = Revolute('d', 0.34,   'a', 0,     'alpha', -pi/2);   % 关节1: ±180度
    L(2) = Revolute('d', 0,      'a', 0,     'alpha',  pi/2);   % 关节2: ±180度
    L(3) = Revolute('d', 0.4,    'a', 0,     'alpha',  pi/2);   % 关节3: ±90度
    L(4) = Revolute('d', 0,      'a', 0,     'alpha', -pi/2, 'qlim', [-pi, pi]);       % 关节4: ±180度
    L(5) = Revolute('d', 0.4,    'a', 0,     'alpha', -pi/2, 'qlim', [-pi/2, pi/2]);   % 关节5: ±90度
    L(6) = Revolute('d', 0,      'a', 0,     'alpha',  pi/2, 'qlim', [-pi/2, pi/2]);   % 关节6: ±90度
    L(7) = Revolute('d', 0.126,  'a', 0,     'alpha', 0,     'qlim', [-pi, pi]);       % 关节7: ±180度
